...
model = PPOActor(hidden_dim=128, n_actions=5).to(device)

loss_fn = torch.nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)

batch_size = 100
dataloader_train = torch.utils.data.DataLoader(H5Dataset(num_chunk=1, h5_path='pups.hdf5'),
                                               shuffle=True, batch_size=batch_size)
dataloader_test = torch.utils.data.DataLoader(H5Dataset(num_chunk=1, h5_path='pups_test.hdf5'),
